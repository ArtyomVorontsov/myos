...
amd_am79c973::amd_am79c973(
    ...
    InterruptManager *interrupts) : Driver(),
                                    InterruptHandler(
                                        interrupts,
                                        dev->interrupt + interrupts->HardwareInterruptOffset()),
                                    ...
                                    busControlRegisterDataPort(dev->portBase + 0x16)
{
    ...
    // 32 bit mode
    registerAddressPort.Write(0x102);
...
uint32_t amd_am79c973::HandleInterrupt(uint32_t esp)
{
    ...
    {
        printf("AMD am79c973 MISSED FRAME ERROR\n");
    }
