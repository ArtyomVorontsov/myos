...
VideoGrapicsArray::VideoGrapicsArray()
    : miscPort(0x3c2),
      ...
      grapicsControllerIndexPort(0x3ce),
      grapicsControllerDataPort(0x3cf),
...
void VideoGrapicsArray::WriteRegisters(uint8_t *registers)
{
    ...
    {
        grapicsControllerIndexPort.Write(i);
        grapicsControllerDataPort.Write(*(registers++));
    }
...
uint8_t *VideoGrapicsArray::GetFrameBufferSegment()
{
    grapicsControllerIndexPort.Write(0x06);
    uint8_t segmentNumber = grapicsControllerDataPort.Read() & (3 << 2);
...
void VideoGrapicsArray::PutPixel(uint32_t x, uint32_t y, uint8_t r, uint8_t g, uint8_t b)
{
